# Profile

<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=26&pause=900&color=7B9EFF&center=true&vCenter=true&width=900&lines=Hi!+I'm+Priya;Welcome+to+my+GitHub+page+:)" />
</h1>

<p align="center">
A brown gal in tech 💻 Serious about data 📈 Even more serious about chai tea lattes 🫖
</p>
<p align="center">
   <img width="50" height="50" alt="image" src="https://imgproxy.attic.sh/insecure/f:png/plain/https://attic.sh/382x6mwx717tjyrsmmu0ra86hj89" />
</p>

## 💁🏽‍♀️ About Me

I enjoy building practical solutions that simplify workflows, improve operational efficiency, and turn complex processes into scalable digital systems.

Currently focused on:
- AI-assisted workflow automation
- Enterprise analytics & reporting
- Process optimisation
- Data-driven solutions
- Digital transformation in tax & finance

My background spans **tax, HR, consulting, and digital delivery**, giving me a strong systems-level view of how data flows through organisations.

## 👩🏽‍💻 What I Build

🧩 **Automation Systems**
- Workflow automation using KNIME & Alteryx
- SQL-powered data pipelines
- Process optimisation for compliance-heavy environments

📊 **Analytics & Insights**
- Power BI dashboards for performance monitoring
- SQL-based reporting & trend analysis
- Data models designed for decision-making

🎨 **Digital & Visual Design**
- Data storytelling through clean, intuitive visuals
- Graphic design & UI thinking applied to analytics
- Bridging analytical depth with creative clarity

## 🧰 Tech Stack

**Data & Analytics:**  
SQL · Power BI · Tableau · KNIME · Alteryx  

**Automation & Systems:**  
Workflow Automation · Data Pipelines · Process Optimisation · Claude

**Digital & Creative:**  
Graphic Design · Data Visualisation · UI Thinking  

**Other:**  
Git · HTML · CSS · Agile Delivery

## 🎓 Certifications (Recent)

- Microsoft Certified: Power BI Data Analyst Associate
- Alteryx Designer Core Certified
- KNIME Advanced Proficiency
- UDemy 100 Days of Code™: The Complete Python Pro Bootcamp

## 🧬 How I Think

- Systems before tools
- Automation before manual effort
- Clarity over complexity
- Design matters — even in data

## ✨ Beyond Tech

I’m passionate about:
- Creativity & self-development
- Digital aesthetics & visual storytelling
- Makeup as a form of creative expression
- Travelling, just to keep life interesting

<p align="center"><em>You're still here? I'll take that as a compliment, thank you! ☺️✨ </em></p>
